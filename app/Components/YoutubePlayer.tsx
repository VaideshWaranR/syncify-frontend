...
export default function YouTubePlayerQueue() {
  ...
  useEffect(() => {
    ...
    return next;
      ...
      console.log("Next song event recieved")
    });
    ...
    socket.on("sync_player", ({ currentTime }) => {
      if (playerRef.current && typeof playerRef.current.seekTo === "function") {
        const diff = Math.abs(
          (playerRef.current.getCurrentTime?.() || 0) - currentTime
        );
        if (diff > 1) {
  ...
  const removeAt = useCallback(
    (idx: number) => {
      setQueue((prev) => {
        ...
        if (idx === currentIndex) {
          const newIndex =
            next.length === 0 ? -1 : Math.min(idx, next.length - 1);
          setCurrentIndex(newIndex);
  ...
  return (
    <Card className="w-full max-w-2xl mx-auto mt-6 shadow-lg rounded-2xl border border-gray-200">
      ...
      <CardContent>
        ...
        <div className="mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button onClick={()=>socket.emit("next")} variant="default">
              Next ▶
            </Button>
          </div>
            ...
            {queue.map((video, idx) => (
              <div
                ...
                className={`queue-item p-4 rounded-xl border shadow-sm bg-card flex items-center justify-between hover:shadow-md transition ${
                  idx === currentIndex
                    ? "cursor-not-allowed ring-2 ring-primary/40 bg-primary/5"
                    : "cursor-move"
                }`}
              >
      ...
      <CardFooter className="text-xs text-muted-foreground flex items-center gap-2">
        <GripVertical className="h-3 w-3" /> Drag items to reorder the queue.
        Current song stays highlighted.
      </CardFooter>
    </Card>
  );
}
